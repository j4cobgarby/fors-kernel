...
static void print_i64(int64_t x, conversion_specifier *spec)
{
    ...
    for (int i = 0; i < PRINTK_I64_BUF_LEN; i++) {
        ...
        if (PRINTK_I64_BUF_LEN - i <= spec->precision || leading_done
...
{
    return c == 'd' || c == 'u' || c == 'x' || c == 'X' || c == 'c' || c == 's'
        || c == 'p' || c == '%';
}
...
void defaults_from_convspec(
    const char c, int *field_width, int *precision, bool *alternate_form)
{
    switch (c) {
    case 'p':
        *precision = 16;
        ...
        case FLAGS:
            if (!char_is_flag(*c)) {
                ...
                case '0':
                    ...
                    break;
                case '-':
                    spec->left_adjusted = true;
                    break;
                case ' ':
                    spec->positive_pad = true;
                    break;
                case '+':
