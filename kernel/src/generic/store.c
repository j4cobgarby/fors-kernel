...
// Get a buffer cache's data buffer. It can then be edited elsewhere, and saved to disk with bc_put
int bc_get(store_id id, size_t addr, char **buf) {
	...
	for (store_id i = 0; i < MAX_STORES; i++) {
		...
		if (!victim->type->wr(&victim->dev, victim->bc_first->addr, victim->bc_first->data)) {
		    printk("[store.c] Could not save victim to disk; looking for another.\n");
		    continue;
		}
	...
	printk("[store.c] Bug? Tried to find a cached block to reuse, but failed to find any.\n");
	return 0;
    } else {
...
// Ensure that a checked-out buffer of addr in a given store is saved back to disk.
